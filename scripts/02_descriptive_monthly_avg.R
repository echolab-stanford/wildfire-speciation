...
monthly <- data %>% select(month_yr, year, month, MF, RC_PM2, NH4, tot_metals, 
                           ammNO3, ammSO4, AL, AS, BR, 
...
#all region, time series of monthly means from 2006-2020
for(i in vars) {
  ...
  g <- ggplot(monthly_gb, aes(x = month_yr, y = get(i))) +
    ...
    ylab(i) +
    xlab("Year") +
    ggtitle("Monthly means, 2006-2020")
  print(g)
