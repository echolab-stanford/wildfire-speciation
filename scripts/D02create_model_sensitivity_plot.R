...
# Description: create a sensitivity plot for different fixed effects and models

# loadd(clean_pm_spec_df, cache = drake::drake_cache("scripts/.drake"))

create_model_sensitivity_plot <- function(clean_pm_spec_df) {
  ...
  # -----------------------------------------------------------------------------
  # 3. RUN DIFFERENT MODELS WITH DIFFERENT FEs
  # -----------------------------------------------------------------------------
  base_reg = feols(c(CR, CU, EC, FE, MN, NI, OC, PB, S, V, ZN)
                          ~ smokePM + nonsmokePM | 
                            monitor_month + year, reg_df) 

  # get coefficients and prepare for plotting
  base_reg_coeffs <- coeftable(base_reg) 
  
  # run regresion accounting for the different monitoring networks that are combined
  mon_source_reg <- feols(c(CR, CU, EC, FE, MN, NI, OC, PB, S, V, ZN)
                        ~ smokePM + nonsmokePM | 
                          monitor_month + Dataset + year, reg_df) 
  
# get coefficients and prepare for plotting
mon_source_coeffs <- coeftable(mon_source_reg) 
