...
for (i in 1:num_simulations) {
  ...
  sample_mod = feols(c(AL,AS,BR, CA, CL, CHL,CR, CU, EC, FE,
                           ...
                           ~ nonsmokePM_MF + smokePM + contrib_daily_structures_destroyed + 
                       contrib_daily_structures_destroyed*smokePM |
                             ID + month + year, sample_df, cluster = 'ID')
  # 3. get coeffs
  ...
  # Plot the points
  plot_coeffs <- ggplot(sample_coeffs, 
    ...
    coord_flip() +
    # facet_wrap(coefficient~factor(species_long, levels = rev(limits)), scales = 'free_y', ncol = 4) +
    theme_minimal() +
