...
# loadd(c(gridded_spec_predictions), cache = drake::drake_cache(".drake"))
...
RSL_chronic <- read_xlsx(file.path(RSL_table_fp), sheet = 4) %>% 
 ...
 # predict the daily concentration of each chemical species
# predict over our sample
# predicted_spec_smoke_conc <- clean_PMspec_df %>% 
#   dplyr::select(Dataset, year, month,region, Date, 
#                 site_id, MF_adj, smokePM, 
#                 nonsmokePM_MF, AL:ZN) %>% 
#   dplyr::select(-SOIL) %>% 
#   pivot_longer(cols = c(AL:ZN), names_to ='species', values_to = 'conc_val') %>% 
#   filter(!is.na(conc_val)) %>% 
#   left_join(regionalPMcoeffs_normalized, by = c('region', 'species')) %>% 
#   # filter to the same species we have toxicity information for
#   filter(species %in% RSL_chronic$species) %>% 
#   mutate(pred_spec_smoke = Estimate*smokePM) %>% 
#   distinct() %>% 
