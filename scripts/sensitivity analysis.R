# # full_sampPM_regMF = feols(c(CR, CU, EC, FE, MN, NI, OC, PB, S, V, ZN,
# #                             # extra vars
# #                             AL,  AS,  BR,  CA,  CL,  K,  `NA`,  NO3, MG, 
# #                             P,  RB,  SE,  SI,
# #                             SR,  SO4 ,  TI,  ZR  
# # )
...
#                 # additional species of interest that have CAS numbers
#                 "AL", "AS", "BR", "CA", "CL", "K", "NA", "NO3","MG", "P", "RB", "SE", "SI",
#                 'SR', 'SO4', "TI", "ZR"),
...
#                      # additional vars
#                      mean(reg_df$AL, na.rm = TRUE), 
#                      mean(reg_df$AS, na.rm = TRUE), 
#                      mean(reg_df$BR, na.rm = TRUE), 
#                      mean(reg_df$CA, na.rm = TRUE), 
#                      mean(reg_df$CL, na.rm = TRUE), 
#                      mean(reg_df$K, na.rm = TRUE),  
#                      mean(reg_df$`NA`, na.rm = TRUE),  
#                      mean(reg_df$NO3, na.rm = TRUE), 
#                      mean(reg_df$P, na.rm = TRUE),  
#                      mean(reg_df$RB, na.rm = TRUE),  
#                      mean(reg_df$SE, na.rm = TRUE),  
#                      mean(reg_df$SI, na.rm = TRUE),  
#                      mean(reg_df$S, na.rm = TRUE),
#                      mean(reg_df$SR, na.rm = TRUE),  
#                      mean(reg_df$SO4, na.rm = TRUE),  
...
#                               monitor_month + year, reg_df) 
#   
#   # full_sampPM_regRCFM = feols(c(CR, CU, EC, FE, MN, NI, OC, PB, S, V, ZN)
...
#   
#   # get coefficients and prepare for plotting
#   # full_sampPM_coeffsRCFM <- coeftable(full_sampPM_regRCFM) %>% 
#   #   rename(pval = 'Pr(>|t|)',
#   #          se = 'Std. Error',
#   #          species = 'lhs',
#   #          pm_type = 'coefficient') %>% 
...
#   #   mutate(measure = 'RCFM')
#   # 
#   # all_coeffs <- bind_rows(full_sampPM_coeffsMF, full_sampPM_coeffsRCFM) %>% 
#   #   left_join(CIs, by = c('species', 'pm_type', 'measure'))
#   
...
#     scale_x_discrete(limits = c( "CR", "PB", "V", "NI", "FE", "CU", "MN","ZN", "S", "OC", "EC",
#                                  # extra vars
#                                  "AL", "AS", "BR", "CA", "CL", "K", "NA", "NO3","MG", "P", "RB", "SE", "SI",
#                                  'SR', 'SO4', "TI", "ZR"
#     )) +
...
#   # save file
#   ggsave(
#     filename = 'Fig2_pct_change_PM_spec_smokeMF.png',
