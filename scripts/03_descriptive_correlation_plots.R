...
df <- data %>% select(SiteCode, year, month, smokePM_pred, RC_PM2, NH4, tot_metals, ammNO3,ammSO4,
...
df[vars] <- log(df[vars])
hist(df$smokePM_pred)
hist(df$NH4)


r2(feols(NH4 ~ smokePM_pred | SiteCode + year + month, data = df), "war2")
