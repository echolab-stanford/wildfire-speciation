...
# mapview(cleaned_spec_df %>% distinct(SiteCode, Longitude, Latitude) %>% st_as_sf(coords = c(x = 'Longitude', y = 'Latitude'), crs = 4326))
...
for(i in 1:length(smoke)){
  ...
  if(nrow(smoke[[i]])>0){
    ...
    st_crs(smoke[[i]]) <- st_crs(aqs_sf) <- NA
  ...
  }else{overlap <- data.frame(SiteCode = aqs_sf$SiteCode, low_count = 0, med_count = 0, high_count = 0, density_missing = 1, smoke_day = 0, geometry  = NA )}
