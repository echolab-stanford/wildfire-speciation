...
avg_ann_conc_df <- pred_conc_df %>%
  ...
  dplyr::summarise(avg_annual_PB = mean(PB_pred_conc, na.rm=TRUE),
                   ...
                   pct95_annual_PB = quantile(PB_pred_conc, .95, na.rm=TRUE),
                   pct95_annual_NI = quantile(NI_pred_conc, .95, na.rm=TRUE),
                   pct95_annual_AS = quantile(AS_pred_conc, .95, na.rm=TRUE),
                   pct5_annual_PB = quantile(PB_pred_conc, .05, na.rm=TRUE),
                   pct5_annual_NI = quantile(NI_pred_conc, .05, na.rm=TRUE),
                   pct5_annual_AS = quantile(AS_pred_conc, .05, na.rm=TRUE)) %>% 
  ungroup() 

write.fst(avg_ann_conc_df, file.path(data_fp, 'clean/avg_annual_AS_PB_NI_conc.fst')) 
...
ann_risk_long_df <- gridded_ann_risk_df %>% 
  ...
  dplyr::select(species, year, mon_yr, canc_burden) %>% 
  # add in UCL
  ...
  pivot_longer(cols = c('pct5_AS_canc_burden', 'pct5_NI_canc_burden', 'pct5_PB_canc_burden'), 
    ...
    mutate(species = str_remove(species, 'pct5_'),
             species = str_remove(species, '_canc_burden')) %>% 
  dplyr::select(species, year, mon_yr, LCL = 'pct5_canc_burden'))
