...
# Description: create different figures for visualizing the data

# ------------------------------------------------------------------------------------
# STEP 1) read in regression dataframe  
# ------------------------------------------------------------------------------------
reg_df <- read.csv(file.path(wip_gdrive_fp, 'intermediate/regression_data.csv')) 

# ------------------------------------------------------------------------------------
# RUN SPECIES BY SPECIES
# ------------------------------------------------------------------------------------
predPM_reg = feols(c(conc_MF, conc_RC_PM2, conc_NH4, conc_tot_metals, 
                     ...
                     conc_ZR) ~ smokePM_pred + non_smokePM_cont | monitor_month + year, reg_df) # add in region,run a model for each region
...
# GET BETAS FOR THREE SPECIES TO START
...
# PLOT FIG 3 EXAMPLE:
  species_frac_plot <- ggplot(all_spec_quantiles_df, 
                          aes(x = quantile, 
  ...
  predPM_reg = feols(c(conc_MF, conc_RC_PM2, conc_NH4, conc_tot_metals, 
                       ...
                       conc_P,conc_PB,conc_RB,conc_S,conc_SE,
                       conc_SI,conc_SOIL,conc_SO4,conc_SR, conc_V,conc_ZN,
                       conc_ZR) ~ smokePM_pred + non_smokePM_cont | monitor_month + year, reg_df, fsplit = ~region) # add in region,run a model for each region
  ...
  # get list of regions:
  region_list <- unique(predPM_coeffs$region)
  # test
  ...
  all_spec_quantiles_df <- purrr::map_df(species_list, function(current_species) {
    ...
    current_weighted_conc_df <- reg_df %>% 
      ...
      dplyr::select(-c(starts_with('region_conc'), State, season, doy, moy, 
                       station_calc_smokePM, station_non_smokePM, monitor_month)) %>% 
      dplyr::select(region, Dataset, SiteCode, Date, region, smoke_day, smokePM_pred, non_smokePM_cont, !!sym(current_species)) %>% 
...
# /Users/ekrasovich/Desktop/ECHOLab Local/wildfire-speciation/figures/Figure 3_total exposure
