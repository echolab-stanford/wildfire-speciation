...
estimating_conc_response_2_burned_structures <- function(burned_struc_smoke_spec_df, parameter_categories) {
  ...
  reg_df <- burned_struc_smoke_spec_df %>% 
    ...
    # filter(smokePM > 0) %>% 
    group_by(ID, year, month, Date, contrib_daily_structures_destroyed, smokePM) %>% 
    dplyr::summarise_at(
    ...
    ungroup() %>% 
    mutate(grid_month = paste0(ID, "-", month)) 
  ...
  spec_ns_samp_avgs_df <- reg_df %>% 
    ...
    sample_mod = feols(c(AL,AS,BR, CA, CL, CHL,CR, CU, EC, FE,
                       ...
                       ~ smokePM + contrib_daily_structures_destroyed + 
                         contrib_daily_structures_destroyed*smokePM |
                         grid_month + year, reg_df, cluster = 'ID')
    # 3. get coeffs
    ...
    full_samp_PMcoeffs_normalized <- sample_coeffs %>% 
    ...
    # Plot the points
    plot_coeffs <- ggplot(full_samp_PMcoeffs_normalized %>% filter(pval <=.05) %>% filter(Estimate > 0),
                          aes(x = species, y = 100*norm_est)) +
      geom_point() +
      geom_linerange(aes(ymin = 100*norm_CI25, 
                         ymax = 100*norm_CI975), stat = "identity") +
      labs(title = "Interaction term coeffs",
           x = "Species",
           y = "% change in concentration relative to nonsmoke day for additional structure burned") + 
    ...
    all_species_conc_results <- map_df(species, function(current_spec) {
      ...
      struc_burned_vec <- seq(0, 25, 1) # projected number of structures burned per day
      #x_vec <- seq(0, 100, 1) # projected smoke concentration
      
      B1 <- current_coeff_df %>% 
        filter(coefficient == 'nonsmokePM_MF') %>% 
        pull(Estimate) 
      ...
      smoke_vals <- unique(reg_df$smoke_bin_avg) # this is just the average smoke PM2.5 in each bin
      ...
      for (sb in struc_burned_vec) {
        for (smoke_val in smoke_vals) {
          partial_eff_structures_burned <- (B3 + B4 * smoke_val)
          y <- sb * partial_eff_structures_burned
          
          y_results <- tibble(SB_val = sb,
                              #avg_smoke_level_in_bin = smoke_val,
                              spec_conc = y,
                              part_eff = partial_eff,
                              B1 =B1,
                              B2 = B2,
                              B3 =B3,
                              B4=B4)
